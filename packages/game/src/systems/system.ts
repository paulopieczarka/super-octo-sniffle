import { ComponentMask, Depth, Dimension, FollowEntity, Position, TargetPosition } from "../components/component";
...
export class WanderAround extends System {
  ...
  public execute(entity: Entity, world: World) {
    ...
    if (followEntity && distance(position, targetPosition) < 1) {
      ...
      const entityPosition = entity ? world.getComponent(entity.id, Position) : undefined;
      
      if (entityPosition) {
        targetPosition.x = entityPosition.x + (Math.random() * followEntity.radius - Math.random() * followEntity.radius);
        targetPosition.y = entityPosition.y + (Math.random() * followEntity.radius - Math.random() * followEntity.radius);
        return;
      }
    }

    if (distance(position, targetPosition) < 1) {
      targetPosition.x = Math.max(0, Math.min(1024, position.x + (Math.random() * 32 - Math.random() * 32)));
      targetPosition.y = Math.max(0, Math.min(1024, position.y + (Math.random() * 32 - Math.random() * 32)));
    }
  }
}
...
export class DepthCalculation extends System {
  public get requiredComponents() {
    return ComponentMask.Position | ComponentMask.Dimension | ComponentMask.Depth;
  }
