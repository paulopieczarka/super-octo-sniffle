  ...
  async fetch(req) {
    ...
    if (pathname === "/ws") {
      const player = observable(
        new Player("Testador200", "red", new Camera(0, 0, 500, 500)),
      );
  ...
  websocket: {
      ...
      for (const socket of game.sockets) {
        socket.send(
          JSON.stringify(
            createAction(
              "addLogMessage",
              `[Server] ${ws.data.player.username} joined @ ${ws.remoteAddress}`,
            ),
          ),
        );
      }
    ...
    message(ws, message: string) {
      ...
      if (action.method === "useCard") {
        const indexToRemove = ws.data.player.cards.findIndex(
        ...
        if (indexToRemove !== -1) {
          ...
          ws.send(
            JSON.stringify(
              createAction("addLogMessage", `[Action] ${card.name} activated`),
            ),
          );
          ws.data.player.cards = ws.data.player.cards.filter(
